...
CSM.ue.sn=CSM.ue_sn;
if(uet){uet("bb",foresterClientScope,{wb:1});
}JSON={};
...
if(!partCount){partCount=1+Math.ceil(size(restData)/availableSizeForDataInEmptyWyw);
}}else{if(MAX_WYW_SIZE-(requiredSizeForData-size(encData))<MIN_DATA_SIZE){return;
}send(sendViaAjax,sendNow);
continue;
}}}if(!groups[msr]){groups[msr]=[];
...
},logWithTimeNonBlocking=function(data,namespace,options,relativeTime,requestId){if(CSM.ue_lnb){w.setTimeout(function(){logWithTime(data,namespace,options,relativeTime,requestId);
},0);
...
},ajaxRequestWorks=1,queue=[],queuePurgerId,browserSupportForCORS=0,xhrVersion,getNewAJAXObject=function(){var newAjaxObj;
...
}},queuePurger=function(){if(!CSM.ue||!CSM.ue.isl){return;
}if(queue.length===0){w.clearInterval(queuePurgerId);
}else{var batchUrl=queue.shift();
sendBatch(batchUrl,1,1);
}},beforeUnloadHandler=function(e){if(!beforeUnloadHandler.executed){send(0,1);
beforeUnloadHandler.executed=1;
}},onloadHandler=function(e){send(1,1);
},prevOnBeforeUnload=w.onbeforeunload,attachAtOnBeforeUnload=function(){if(!window.chrome){w.onbeforeunload=function(e){if(prevOnBeforeUnload){prevOnBeforeUnload(e);
}beforeUnloadHandler(e);
};
}else{ue.attach("beforeunload",beforeUnloadHandler);
ue.ulh.push(beforeUnloadHandler);
}};
attachAtOnBeforeUnload();
if(CSM.ue.attach){CSM.ue.attach("load",onloadHandler);
...
}if(w.P&&w.P.register){w.P.register("forester-client",function(){});
}if(uex){uex("ld",foresterClientScope,{wb:1});
}})(ue_csm,window);
